...
@Component({standalone:true,imports:[ReactiveFormsModule],template:`
  ...
  <section class="account-layout"><article class="card profile-card"><span class="avatar" aria-hidden="true">{{initials()}}</span><div><span class="eyebrow">Conta institucional</span><h2>{{auth.user()?.name}}</h2><p>{{auth.user()?.email}}</p></div><dl><div><dt>Perfis</dt><dd>@for(role of roleLabels();track role){<span class="badge badge--info">{{role}}</span>}</dd></div><div><dt>Campus</dt><dd>{{campusName()}}</dd></div></dl></article>
...
export class AccountComponent implements OnInit {
  ...
  readonly campusName=signal('Sem campus vinculado');
  readonly roleLabels=computed(()=>this.auth.user()?.roles.map(labelFor)??[]);
  ...
  ngOnInit():void{
    const campusId=this.auth.user()?.campusId;
    if(!campusId)return;
    this.campusName.set('Carregando campus…');
    this.api.campuses().subscribe({
      next:campuses=>this.campusName.set(campuses.find(c=>c.id===campusId)?.name??'Campus vinculado'),
      error:()=>this.campusName.set('Campus vinculado')
    });
  }

  initials(){return(this.auth.user()?.name??'').split(' ').filter(Boolean).slice(0,2).map(v=>v[0]).join('').toUpperCase();}
  changePassword(){const v=this.form.getRawValue();this.form.markAllAsTouched();if(this.form.invalid)return;if(v.newPassword!==v.confirmation){this.message.set('A confirmação deve ser igual à nova senha.');return;}this.message.set('');this.busy.set(true);this.auth.changePassword(v.currentPassword,v.newPassword).subscribe({next:()=>this.busy.set(false),error:()=>{this.busy.set(false);this.message.set('Não foi possível alterar a senha. Confira a senha atual.');}});}
  renew(){this.busy.set(true);this.auth.refresh().subscribe({next:()=>{this.busy.set(false);this.toast.show('Sessão renovada com segurança.','success');},error:()=>{this.busy.set(false);this.toast.show('Entre novamente para renovar a sessão.','error');}});}
}
